...
The parameters are the same for every machine, so the command set of a machine that has never been captured can be computed from its identifier bytes alone. This component computes the checksum for every outgoing message (see [checksum.h](components/philips_coffee_machine/checksum.h)) rather than relying on pre-captured byte tables, and verifies it on every incoming mainboard status message before processing it.

Credit for discovering this goes to [Thies Gerken](https://github.com/thiesgerken) and Fable, see [TillFleisch/ESPHome-Philips-Smart-Coffee#123](https://github.com/TillFleisch/ESPHome-Philips-Smart-Coffee/pull/123).
...
Set both button bits and compute the checksum as described in [Checksum](#checksum). Untested on hardware.
